...
def busca_valor(m, x):
    i= 0
    j= 0
    while (i<size) and (m[i,j]!= x):
        while (j<size) and (m[i,j]!= x):
            j=j+1
        if j!=size and m[i,j]==x:
            break
        else:
            j=0
            i=i+1
...
def heuristica(m):
    ...
    for i in range(0, size-1):
        for j in range(0, size-1):
            if m[i,j] != saida_esperada[i,j]:
                a, b = busca_valor(saida_esperada, m[i,j])
                h = abs(i-a) + abs(j-b)            
    return h
...
def acceptable_runtime(start_time, nodos):
		...
		if time.time()-start_time >= 3600:
			print("\nBusca A* com heuristica da distância de Manhatan\n")
...
if __name__ == '__main__':
	...
	while lista_dados and acceptable_runtime(ini, nos_abertos):
		...
		#print(type(matriz.data))
		#print(type(saida_esperada))
